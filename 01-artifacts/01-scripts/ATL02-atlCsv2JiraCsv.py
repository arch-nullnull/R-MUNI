...
#
# Zweck (Flow-Stage):
# - atl_*.csv aus 02-stages/00-archimatearchive/ lesen
# - Eine Jira-Import-CSV generieren (alle Layer zusammengeführt)
# - Output (temporär, Import-Fragment):
#     <rootfolder>/02-stages/00-archimatearchive/jira_ea_import.csv
# - Log:
#     <rootfolder>/02-stages/99-logs/ATL02-atlCsv2JiraCsv.log
#
# Jira-Felder:
...
IN_DIR_REL   = os.path.join("02-stages", "00-archimatearchive")
OUT_DIR_REL  = os.path.join("02-stages", "00-archimatearchive")
LOGS_DIR_REL = os.path.join("02-stages", "99-logs")
...
def resolve_root(script_dir: str) -> str:
    logs_candidate = os.path.abspath(
        os.path.join(script_dir, "..", "..", "02-stages", "99-logs")
    )
...
def main() -> None:
    ...
    print(
        ...
        f"02-stages/00-archimatearchive/{OUTPUT_FILENAME}"
    )
