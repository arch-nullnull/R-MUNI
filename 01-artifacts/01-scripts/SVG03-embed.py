...
# Aufgabe : Bilddateien als eingebettete Raster-SVG konvertieren (Option A)
# Output  : 02-stages\99-logs\SVG03-embed.log
# Abhängig: HLP00_resolve_root.py, SVG01-inventory.log
# ==============================================================================
# Das Rasterbild wird via Inkscape CLI als Base64-Objekt in eine SVG-Hülle
# verpackt — kein Vektorisierungsschritt, kein Qualitätsverlust.
# Inkscape CLI: inkscape.exe "<quelle>" --export-type=svg --export-filename="<ziel>"
# ==============================================================================
...
try:
    ...
    print("[SVG03] FEHLER: HLP00_resolve_root.py nicht gefunden.")
    sys.exit(1)
...
def parse_inventar(inventar_path):
    ...
    die konvertiert werden sollen (Status OK oder RENAME_REQUIRED).
    Verwendet feste Spaltenposition wie in SVG02."""
    ergebnis = []
    STATUS_START    = 2
...
def main():
    ...
    log_lines.append("=" * 72)
    log_lines.append(f"  {SCRIPT_ID} — Embed-Konvertierung (Option A)")
    ...
    # --- 5. Konvertierung ---
    log_lines.append("--- 5. Konvertierung ---")
    log_lines.append("")
    ...
    for inv_status, dateiname in inventar:
        ...
        # Quelldatei prüfen — bei RENAME_REQUIRED könnte sie umbenannt worden sein
        ...
        # Zieldateiname: Dateiname ohne Endung + .svg
        ...
        # Inkscape CLI aufrufen
        try:
            result = subprocess.run(
                [inkscape_exe, src_path,
                 "--export-type=svg",
            ...
            zeile = f"  {dateiname:<40} {svg_name:<40} {'FEHLER':<8} Timeout (>60s)"
            log_lines.append(zeile)
            print(zeile)
            zaehler["FEHLER"] += 1
        except Exception as e:
    ...
    if zaehler["FEHLER"] > 0:
        ...
        log_lines.append(f"           Log prüfen. SVG05 für vollständigen Abgleich ausführen.")
