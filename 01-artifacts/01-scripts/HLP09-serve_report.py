...
#   Liest <rootfolder> aus root.cfg.
#   Liest Report-Konfiguration aus:
#     01-artifacts/05-reports/webconfig.txt
#
...
#   STRG+C im Terminal
#
# Ablage:  01-artifacts/01-scripts/HLP09-serve_report.py
...
SCRIPT_NAME    = "HLP09"
REPORTS_REL    = os.path.join("01-artifacts", "05-reports")
WEBCONFIG_REL  = os.path.join("01-artifacts", "05-reports", "webconfig.txt")
LOG_REL        = os.path.join("02-stages", "99-logs", "HLP09-serve_report.log")
...
def find_root_txt(script_dir: str) -> str:
    """
    Sucht root.cfg relativ zum Script-Verzeichnis.
    Script liegt in: 01-artifacts/01-scripts/
    root.cfg liegt in: <rootfolder>/root.cfg
    Zwei Ebenen hoch: ../../root.cfg
    """
    return os.path.abspath(os.path.join(script_dir, "..", "..", "root.cfg"))


def read_blueprint_root(root_txt_path: str, log_path: str | None) -> str:
    """Liest <rootfolder> aus root.cfg und loest den Pfad auf."""
    if not os.path.isfile(root_txt_path):
        die(f"root.cfg nicht gefunden: {root_txt_path}", log_path)
        ...
        for line in f:
            ...
            if stripped.startswith("<rootfolder>="):
                if root_value is not None:
                    die("Mehrere <rootfolder> Eintraege in root.cfg gefunden", log_path)
                root_value = stripped.split("=", 1)[1].strip()

    if root_value is None:
        die("Kein <rootfolder> Eintrag in root.cfg gefunden", log_path)
    if root_value == "":
        die("<rootfolder> ist leer in root.cfg", log_path)

    # Relativen Pfad gegen Position von root.cfg aufloesen
    ...
    if not os.path.isdir(root_value):
        die(f"<rootfolder> Verzeichnis existiert nicht: {root_value}", log_path)
...
def main():
    ...
    # root.cfg finden und Blueprint-Root aufloesen
