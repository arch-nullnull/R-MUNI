...
#     <rootfolder>/02-stages/99-logs/XML00-root.resolved.txt
# - Reads source folder declarations from:
#     <rootfolder>/01-artifacts/00-xml/child_mapping.txt
...
#
# Reference artifacts:
#   <rootfolder>/02-stages/00-archimatearchive/XML01-sources.resolved.txt
#   <rootfolder>/02-stages/01-bpmnarchive/XML01-sources.resolved.txt
#
# Logs:
#   <rootfolder>/02-stages/99-logs/XML01-sources.log
...
def archive_dir_for(root, source_type):
    ...
    path = os.path.join(root, "02-stages", sub)
...
def main():
    ...
    mapping_file = os.path.join(root, "01-artifacts", "00-xml", "child_mapping.txt")
