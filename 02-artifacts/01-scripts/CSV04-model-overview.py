...
#
# Output:
# - <rootfolder>/03-stages/run-scope.txt (extended, not interpreted)
# - <rootfolder>/03-stages/99-logs/CSV04-model_overview.log
#
# Rules:
# - Read-only
...
def main():
    ...
    stages_dir = os.path.join(root, "03-stages")
    ...
    # --- OEF artifacts ---
    oef_dir = os.path.join(root, "02-artifacts", "00-xml", "03-child", "00-archimatechild")
    for f in list_files(oef_dir):
        entries.append(("OEF", f))
        log(f"Found OEF artifact: {f}", log_path)

    # --- XLSX artifacts ---
    xlsx_dir = os.path.join(root, "02-artifacts", "03-XLSX", "03-child", "00-archimatechild")
    for f in list_files(xlsx_dir):
        entries.append(("XLSX", f))
        log(f"Found XLSX artifact: {f}", log_path)

    # --- CSV artifacts ---
    csv_dir = os.path.join(root, "02-artifacts", "02-csv", "03-child", "00-archimatechild")
